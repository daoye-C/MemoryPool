...
void testBasicAllocation() 
{
    ...
    // 测试小内存分配
    void* ptr1 = MemoryPool::mempool::allocate(8);
    assert(ptr1 != nullptr);
    MemoryPool::mempool::deallocate(ptr1, 8);

    // 测试中等大小内存分配
    void* ptr2 = MemoryPool::mempool::allocate(1024);
    assert(ptr2 != nullptr);
    MemoryPool::mempool::deallocate(ptr2, 1024);

    // 测试大内存分配（超过MAX_BYTES）
    void* ptr3 = mempool::allocate(1024 * 1024);
    assert(ptr3 != nullptr);
    mempool::deallocate(ptr3, 1024 * 1024);
        ...
        try 
        {
            ...
            for (int i = 0; i < ALLOCS_PER_THREAD && !has_error; ++i) 
            {
                ...
                if (rand() % 2 && !allocations.empty()) 
                {
                    ...
                    mempool::deallocate(allocations[index].first, 
...
void testEdgeCases() 
{
    ...
    // 测试0大小分配
    void* ptr1 = mempool::allocate(0);
    assert(ptr1 != nullptr);
    mempool::deallocate(ptr1, 0);
    
    // 测试最小对齐大小
    void* ptr2 = mempool::allocate(1);
    assert(ptr2 != nullptr);
    assert((reinterpret_cast<uintptr_t>(ptr2) & (ALIGNMENT - 1)) == 0);
    mempool::deallocate(ptr2, 1);
    
    // 测试最大大小边界
    void* ptr3 = mempool::allocate(MAX_BYTES);
    assert(ptr3 != nullptr);   // 括号内的是不应该出现的情况，出现则抛出断言
    mempool::deallocate(ptr3, MAX_BYTES);
    
    // 测试超过最大大小
    void* ptr4 = mempool::allocate(MAX_BYTES + 1);
    assert(ptr4 != nullptr);
    mempool::deallocate(ptr4, MAX_BYTES + 1);
