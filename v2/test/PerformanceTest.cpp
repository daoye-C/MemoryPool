...
public:
    ...
    static void warmup() 
    {
        ...
        for (int i = 0; i < 1000; ++i) 
        {
            for (size_t size : {32, 64, 128, 256, 512}) {
                void* p = MemoryPool::mempool::allocate(size);
        ...
        {
            MemoryPool::mempool::deallocate(ptr, size);  // 使用实际分配的大小进行释放
        }
    ...
    static void testSmallAllocation() 
    {
        ...
        // 测试内存池
        {
            ...
            {
                ptrs.push_back(MemoryPool::mempool::allocate(SMALL_SIZE));
                
                // 模拟真实使用：部分立即释放
                if (i % 4 == 0) 
                {
                    MemoryPool::mempool::deallocate(ptrs.back(), SMALL_SIZE);
                    ptrs.pop_back();
                }
            }
            ...
            for (void* ptr : ptrs) 
            {
                MemoryPool::mempool::deallocate(ptr, SMALL_SIZE);
            }
        ...
        // 测试new/delete
        {
            ...
            for (void* ptr : ptrs) 
            {
                delete[] static_cast<char*>(ptr);  // 应该是需要一个类型才能释放
            }
    ...
    static void testMultiThreaded() 
    {
        ...
        auto threadFunc = [](bool useMemPool) 
        {
            ...
            {
                size_t size = dis(gen);
                void* ptr = useMemPool ? MemoryPool::mempool::allocate(size) 
                                     : new char[size];
                ...
                {  // 75%的概率释放
                    ...
                    if (useMemPool) 
                    {
                        MemoryPool::mempool::deallocate(ptrs[index].first, ptrs[index].second);
                    } 
                    else 
                    {
                        delete[] static_cast<char*>(ptrs[index].first);
                    }
                    ptrs[index] = ptrs.back();  // 删除掉该项，通过 “等价变换”  index 只索引不具有其他意义
                    ptrs.pop_back();
                }
            }
            ...
            for (const auto& [ptr, size] : ptrs) 
            {
                if (useMemPool) 
                {
                    MemoryPool::mempool::deallocate(ptr, size);
                } 
                else 
                {
        ...
        // 测试内存池
        {
            ...
            for (size_t i = 0; i < NUM_ALLOCS; ++i) 
            {
                ...
                if (i % 100 == 0 && !ptrs.empty()) 
                {
                    ...
                    {
                        MemoryPool::mempool::deallocate(ptrs.back().first, ptrs.back().second);
                        ptrs.pop_back();
                    }
                }
            }
